...
## Temporary QA logins (2026-09-20) — REMOVED
`qa_accounts_tmp@vhc.in` and `qa_employer_tmp@vhc.in` were created to prove role scoping
(Accounts sees everything, an employer sees only their branch) and **deleted after testing**.
Recreate any time with `python3 -m scripts.qa_role_users create` (password `QaRevenue@2026`,
the QA employer becomes a co-manager of the Faridabad team) and remove with
`python3 -m scripts.qa_role_users remove`.
