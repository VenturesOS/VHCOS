...
16. ~~**(P0)** Memory leak Step 1 patch — gc.collect() + malloc_trim(0)~~ 🟡 **SHIPPED 2026-02-27 (awaiting EC2 deploy + verification)** — py-spy dump on PID 43584 (3.7 GB RSS / 4.4 GB VmData) revealed the leak lives in glibc secondary arenas, not thread explosion or socket churn. Added `gc.collect() + libc.malloc_trim(0)` to a new `_release_memory()` helper called from a `finally` block at the end of `_background_full_groq_enrich`. Locally verified `malloc_trim(0)` returns 1 (memory actually released). Awaiting `git pull` on EC2 to confirm worker RSS drops below 1.5 GB. Code: `routes/extension.py:13-14, 956-995, 1351-1356`.
