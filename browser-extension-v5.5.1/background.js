...
/**
 * Fuzzy matches two candidate names to see if they might be the same person.
 */
function fuzzyNameMatch(name1, name2) {
  ...
  const clean = (name) => {
    return name
      .toLowerCase()
      .replace(/\b(mr|ms|mrs|dr|shri|smt|prof)\.?\s+/gi, '') // strip salutations
  ...
  // If one is sub-string of another and length is reasonable (e.g. > 5 chars)
...
/**
 * Checks a list of candidates against the local captureHistory.
 */
function checkLocalHistory(candidates, history) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    for (const hist of history) {
      ...
      if (fuzzyNameMatch(cand.name, hist.name)) {
        found = hist;
        break;
      }
    }
    ...
    if (found) {
      results.push({
        ...
        captured_at: found.timestamp,
        match_confidence: matchedByUrl ? 'high' : 'medium'
      });
    } else {
      results.push({
        index: i,
