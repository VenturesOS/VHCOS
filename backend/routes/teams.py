...
async def get_teams(
    ...
    - Soft-deleted teams are hidden by default (pass include_deleted=true to see them)
    Dynamically resolves company_names from company_ids to stay in sync.
    """
    query = {}
    if current_user["role"] == "employer":
        query["employer_id"] = current_user["id"]
    if not include_deleted:
        # Exclude soft-deleted teams. Missing `status` is treated as active.
...
async def get_team(team_id: str, current_user: dict = Depends(require_role(["admin", "employer"]))):
    """Get a specific team by ID. Resolves company names dynamically."""
