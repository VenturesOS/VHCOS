...
@router.put("/trackers/{tracker_id}/columns")
async def reorder_tracker_columns(tracker_id: str, columns: List[dict], user=Depends(require_role(["admin", "recruiter", "employer"]))):
    """Update column order for a tracker. Accepts the full columns array in new order."""
    tracker = await db.submission_trackers.find_one({"id": tracker_id}, {"_id": 0})
    if not tracker:
        raise HTTPException(status_code=404, detail="Tracker not found")
    # Validate: all existing column keys must be present
    existing_keys = {c["key"] for c in tracker.get("columns", [])}
    new_keys = {c["key"] for c in columns}
    if existing_keys != new_keys:
        raise HTTPException(status_code=400, detail="Column keys must match existing columns")
    await db.submission_trackers.update_one(
        {"id": tracker_id},
...
async def upload_tracker_data(
    ...
    {h.strip().lower().replace(" ", "_").replace("/", "_"): i for i, h in enumerate(headers)}
