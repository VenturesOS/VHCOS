...
async def get_attendance_analytics(
    ...
    user=Depends(require_role(["admin", "employer", "accounts"])),
):
    """
...
async def _detect_patterns(records, user_ids, user_map, working_days, settings):
    ...
    insights = []
    settings.get("late_threshold_minutes", 15)
    ...
    for uid in user_ids:
        ...
        sum(1 for r in user_recs if r.get("status") in ("present", "wfh"))
...
async def mark_notification_read(event_id: str, user=Depends(require_role(["admin", "recruiter", "employer", "accounts"]))):
...
async def mark_all_read(user=Depends(require_role(["admin", "recruiter", "employer", "accounts"]))):
...
async def trigger_reminders(user=Depends(require_role(["admin", "accounts"]))):
...
async def trigger_auto_absent(user=Depends(require_role(["admin", "accounts"]))):
