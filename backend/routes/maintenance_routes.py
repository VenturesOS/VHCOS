...
# ══════════════════════════════════════════════════════
# API Metrics Dashboard Endpoints
# ══════════════════════════════════════════════════════

TIME_RANGE_MAP = {"1h": 3600, "6h": 21600, "24h": 86400, "7d": 604800}

@maintenance_router.get("/api-metrics/overview")
async def api_metrics_overview(range: str = "1h", user=Depends(require_role("admin"))):
    """Get API metrics overview: total requests, error rate, avg response time."""
    from middleware.api_metrics import get_api_overview
    seconds = TIME_RANGE_MAP.get(range, 3600)
    return await get_api_overview(seconds)


@maintenance_router.get("/api-metrics/timeseries")
async def api_metrics_timeseries(range: str = "1h", user=Depends(require_role("admin"))):
    """Get time-series data for charts: error rate and response time over time."""
    from middleware.api_metrics import get_timeseries
    seconds = TIME_RANGE_MAP.get(range, 3600)
    buckets = {"1h": 30, "6h": 36, "24h": 48, "7d": 42}.get(range, 30)
    return await get_timeseries(seconds, buckets)


@maintenance_router.get("/api-metrics/top-endpoints")
async def api_metrics_top_endpoints(range: str = "1h", sort: str = "errors", user=Depends(require_role("admin"))):
    """Get top endpoints by error count or slowest response time."""
    from middleware.api_metrics import get_top_endpoints
    seconds = TIME_RANGE_MAP.get(range, 3600)
    return await get_top_endpoints(seconds, sort)


@maintenance_router.get("/api-metrics/db-health")
async def api_metrics_db_health(user=Depends(require_role("admin"))):
    """Get MongoDB connection pool stats and health."""
    from middleware.api_metrics import get_db_health
