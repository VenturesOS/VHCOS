...
from services.job_service import generate_job_public_id
...
async def create_job(job_data: JobCreate, current_user: dict = Depends(require_role(["admin", "employer", "recruiter"]))):
    ...
    if company_id and company_id != "default":
        ...
        if company:
            ...
            # Note: Company assignment checks removed — any recruiter/employer can create jobs for any company.
            # Account manager assignment is for visibility/management, not job creation restriction.
    
    # Determine initial status based on role
    # All roles create active jobs (auto-approved)
    ...
    if current_user["role"] == "recruiter":
        ...
        # Find the first active team for this employer (for team_id assignment)
        team = await db.teams.find_one({"employer_id": current_user["id"], "status": "active"}, {"_id": 0})
        if not team:
            # Also check teams without status field
    ...
    job_doc = {
        ...
        **job_data_dict,
        ...
        "assigned_recruiter_id": current_user["id"] if current_user["role"] == "recruiter" else None,
        "assigned_recruiter_name": current_user["name"] if current_user["role"] == "recruiter" else None,
    ...
    # Notify assigned recruiters about the new job
    for rec_id in job_doc.get("assigned_recruiters", []):
        try:
            await create_notification(
                user_id=rec_id,
                notification_type="job_assigned",
                title="New Job Assigned",
                message=f"You've been assigned to: {job_doc.get('title', 'New Job')}",
                link="/recruiter/jobs",
...
async def get_jobs(
    ...
    """List jobs. sort_order: 'newest' (default) or 'oldest'."""
    query = {}
    
    if current_user["role"] == "employer":
        teams_cursor = db.teams.find({"employer_id": current_user["id"]}, {"_id": 0, "id": 1})
    ...
    sort_dir = 1 if sort_order == "oldest" else -1
    jobs = await db.jobs.find(query, {"_id": 0}).sort("created_at", sort_dir).to_list(1000)

    # Enrich with posted_by_name and assigned_recruiter_names
    user_ids = set()
    for j in jobs:
        if j.get("posted_by"):
            user_ids.add(j["posted_by"])
        for rid in (j.get("assigned_recruiters") or []):
            user_ids.add(rid)
    if user_ids:
        users_cursor = db.users.find({"id": {"$in": list(user_ids)}}, {"_id": 0, "id": 1, "name": 1})
        user_map = {u["id"]: u.get("name", "Unknown") async for u in users_cursor}
        for j in jobs:
            j["posted_by_name"] = user_map.get(j.get("posted_by"), None)
            recruiter_ids = j.get("assigned_recruiters") or []
            j["assigned_recruiter_names"] = [user_map.get(rid, "Unknown") for rid in recruiter_ids if rid in user_map]
...
@jobs_router.get("/jobs/{job_id}/suggestions")
async def get_job_suggestions(
    job_id: str,
    current_user: dict = Depends(get_current_user),
):
    """Get AI-generated candidate suggestions for a job."""
    job = await db.jobs.find_one({"id": job_id}, {"_id": 0, "id": 1})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")

    doc = await db.job_suggestions.find_one({"job_id": job_id}, {"_id": 0})
    if not doc:
        return {"status": "not_started", "suggestions": [], "total_matched": 0}

    # Filter out candidates already linked to this job
    already_linked = set()
    apps = db.applications.find(
        {"job_id": job_id}, {"_id": 0, "candidate_id": 1}
    )
    async for app in apps:
        already_linked.add(app.get("candidate_id"))

    filtered = [
        s for s in doc.get("suggestions", [])
        if s.get("candidate_id") not in already_linked
    ]
    
    filtered_maybes = [
        s for s in doc.get("maybe_candidates", [])
        if s.get("candidate_id") not in already_linked
    ]
    ...
    return {
        ...
        "maybe_candidates": filtered_maybes,
        "total_matched": doc.get("total_matched", 0),
        "total_maybes": doc.get("total_maybes", 0),
        "total_scored": doc.get("total_scored", 0),
        "elapsed_seconds": doc.get("elapsed_seconds"),
    }


@jobs_router.post("/jobs/{job_id}/suggestions/refresh")
async def refresh_job_suggestions(
    job_id: str,
    current_user: dict = Depends(require_role(["admin", "employer", "recruiter"])),
):
    """Re-trigger background matching for a job."""
    job = await db.jobs.find_one({"id": job_id}, {"_id": 0})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")

    from services.job_suggestions import generate_job_suggestions
    asyncio.create_task(generate_job_suggestions(job, db))

    return {"message": "Suggestion refresh started", "status": "processing"}
...
async def delete_job(job_id: str, current_user: dict = Depends(require_role(["admin", "employer", "recruiter"]))):
    job = await db.jobs.find_one({"id": job_id}, {"_id": 0})
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    # Recruiters can only delete jobs they created
    if current_user["role"] == "recruiter" and job.get("posted_by") != current_user["id"]:
...
async def update_mandate_shareable_link(
    ...
    Rules:
    - Admin has full access
    - Employer can control jobs under their companies/team
    - Recruiter can control jobs they created or are assigned to
    ...
    # Unambiguous character set: no i/l/1/0/O/o that look similar in many fonts
    CLEAR_CHARS = 'ABCDEFGHJKMNPQRSTUVWXYZabcdeghjkmnpqrstuvwxyz23456789'
    
    def generate_clear_token(length=32):
        return ''.join(secrets.choice(CLEAR_CHARS) for _ in range(length))
    ...
    # Access control
    if current_user["role"] == "recruiter":
        assigned = job.get("assigned_recruiters", [])
        if job.get("posted_by") != current_user["id"] and current_user["id"] not in assigned:
            raise HTTPException(status_code=403, detail="Access denied")
    elif current_user["role"] == "employer":
        team = await db.teams.find_one({"employer_id": current_user["id"], "status": "active"}, {"_id": 0})
        if team:
    ...
    # Generate secure token if enabling (always regenerate for fresh link)
    mandate_share_token = job.get("mandate_share_token")
    if update.enabled:
        mandate_share_token = generate_clear_token(32)
...
Return ONLY valid JSON, no markdown or explanation."""
        ...
        response = await _llm_chat(
            "You are an expert job description parser. Extract structured data accurately.",
            parse_prompt
        )
    ...
    await db.jobs.update_one(
        {"id": job_id},
        {
            "$set": {
                ...
                "assigned_recruiter_id": recruiter_ids[0] if recruiter_ids else None,
                "assigned_recruiter_name": recruiter_details[0]["name"] if recruiter_details else None,
                "updated_at": now
            },
