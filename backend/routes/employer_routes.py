...
# ============== EMPLOYER: SELF-ADD COMPANY ==============

def _validate_commercial(commercial: CommercialModel):
    """Validate commercial model fields."""
    if commercial.type == "percentage" and not commercial.percentage_value:
        raise HTTPException(status_code=400, detail="Percentage value is required for percentage type")
    if commercial.type == "fixed" and not commercial.fixed_fee_amount:
        raise HTTPException(status_code=400, detail="Fixed fee amount is required for fixed type")
    if commercial.type == "level_based" and (not commercial.level_config or len(commercial.level_config) == 0):
        raise HTTPException(status_code=400, detail="At least one level range is required for level-based type")
    if commercial.type == "fixed_level_based" and (not commercial.fixed_fee_levels or len(commercial.fixed_fee_levels) == 0):
        raise HTTPException(status_code=400, detail="At least one fixed fee level is required for fixed level-based type")


@employer_router.post("/employer/companies", response_model=CompanyResponse)
async def employer_create_company(
    company_data: CompanyCreate,
    current_user: dict = Depends(require_role(["employer"]))
):
    """
    Employer self-creates a company. Auto-approved and auto-assigned to the employer.
    """
    ...
    company = {
        ...
        "assigned_employer_name": current_user.get("name"),
        "status": "active",
        "created_at": now,
    ...
    # Auto-assign company to employer's team
    existing_team = await db.teams.find_one({"employer_id": current_user["id"], "status": "active"}, {"_id": 0})
    if existing_team:
        await db.teams.update_one(
            {"id": existing_team["id"]},
            {
                "$addToSet": {"company_ids": company["id"], "company_names": company["name"]},
                "$set": {"updated_at": now}
            }
        )
    else:
        await db.teams.insert_one({
            "id": str(uuid.uuid4()),
            "name": f"{current_user.get('name', 'Employer')} Team",
...
async def get_employer_team_with_metrics(current_user: dict = Depends(require_role(["employer"]))):
    ...
    for recruiter in recruiters:
        ...
        stages = {
            "sourced": 0, "applied": 0, "shortlisted": 0, "submitted_to_client": 0, "interview": 0,
...
async def get_employer_companies_with_details(current_user: dict = Depends(require_role(["employer", "recruiter"]))):
    """
    Get assigned companies with commercial details, mandates, and pipelines.
    Recruiters see companies from their team.
    """
    # Find team — for employer via employer_id, for recruiter via recruiter_ids
    ...
    for company in companies:
        ...
        for job in company_jobs:
            ...
            stages = {"sourced": 0, "applied": 0, "shortlisted": 0, "submitted_to_client": 0, "interview": 0, "offered": 0, "hired": 0, "joined": 0, "rejected": 0}
...
async def get_employer_pipeline(
    ...
    if not team:
        ...
        return {
            "pipeline": {stage: [] for stage in _empty},
            "stage_counts": {stage: 0 for stage in _empty},
    ...
    for app in applications:
        ...
        # Legacy remap for apps created before the simplified pipeline
        if stage == "applied":
            stage = "sourced"
...
# NOTE: assign-employer endpoint is in admin.py only (removed duplicate here to avoid route collision)
