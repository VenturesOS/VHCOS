...
async def generate_maintenance_report() -> bytes:
    ...
    # Count unrecovered IN THE SAME PERIOD as the table above so the header matches
    # the rows the reader is looking at. A document is "unrecovered" if `is_recovered`
    # is missing or not literally True (covers None, missing, and False values).
    unrecovered_in_period = await db.naukri_capture_logs.count_documents({
        "status": "failed",
    ...
    story.append(Paragraph(
        f"Naukri Extension — Failed Captures ({len(failed_captures)} failed / "
        f"{total_captures_period} total, {unrecovered_in_period} unrecovered in period)",
        styles["SectionTitle"],
    ))
    if failed_captures:
        fc_rows = []
