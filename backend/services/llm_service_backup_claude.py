...
async def chat_completion(
    ...
    """Waterfall LLM call: Direct Anthropic (rotated keys) -> OpenRouter Free -> Claude (Emergent) -> Direct OpenAI.
    ...
    # ── 0. Direct Anthropic SDK with key rotation (FREE tier) ──
    import anthropic as _anthropic

    _ant_keys = []
    _pk = os.environ.get("ANTHROPIC_API_KEY")
    if _pk:
        _ant_keys.append(_pk)
    for _i in range(2, 5):
        _k = os.environ.get(f"ANTHROPIC_API_KEY_{_i}")
        if _k:
            _ant_keys.append(_k)

    if _ant_keys:
        for idx, key in enumerate(_ant_keys, 1):
            try:
                ...
                if content and content.strip():
                    logger.warning(f"[LLM] Direct Anthropic key#{idx} | tokens: in={resp.usage.input_tokens} out={resp.usage.output_tokens}")
                    from services.bedrock_service import _track_key_usage
                    await _track_key_usage(idx, resp.usage.input_tokens, resp.usage.output_tokens, "llm")
                    if return_usage:
                        return {"content": content, "usage": {"input": resp.usage.input_tokens, "output": resp.usage.output_tokens}, "model": "claude-haiku-4-5"}
                    return content
            except (_anthropic.AuthenticationError, _anthropic.RateLimitError) as e:
                logger.warning(f"[LLM] Anthropic key#{idx} failed: {e}")
                errors.append(f"Anthropic#{idx}: {e}")
                continue
            except _anthropic.APIStatusError as e:
                if e.status_code in (400, 401, 403, 429):
                    logger.warning(f"[LLM] Anthropic key#{idx} status {e.status_code}: {e}")
                    errors.append(f"Anthropic#{idx}: {e}")
                    continue
                raise
            except Exception as e:
                errors.append(f"Anthropic#{idx}: {e}")
                logger.warning(f"[LLM] Anthropic key#{idx} error: {e}")
                continue
        logger.warning(f"[LLM] All {len(_ant_keys)} Anthropic keys exhausted — trying OpenRouter/Emergent")
