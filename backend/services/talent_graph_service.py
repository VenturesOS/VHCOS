...
The 3-line summary is generated by the approved LLM chain
...
# ──────────────────────────────────────────────────────────────────────────────
# Summary generation (approved LLM chain → free)
# ──────────────────────────────────────────────────────────────────────────────

async def generate_candidate_summary(candidate: Dict[str, Any]) -> Optional[str]:
    """Generate a 3-line TL;DR via the approved LLM chain. Returns None on failure
    ...
    try:
        ...
        resp = await call_llm_chain(system_prompt, user_prompt, temperature=0.2, max_tokens=500)
        ...
        if len(summary) < 60 or summary.count(" ") < 8:
            logger.warning(f"[TalentGraph] LLM summary too short/junk: {summary[:80]!r}")
            return None
...
def fallback_rule_summary(candidate: Dict[str, Any]) -> str:
    """Cheap rule-based 3-liner used when LLM is unavailable."""
...
async def upsert_candidate_embedding(
    ...
    *,
    ...
    Args:
        ...
        summary: If provided, persist as-is. If None, generate via LLM with
