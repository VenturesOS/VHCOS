...
def scan_for_threats(content: bytes, filename: str) -> list:
    ...
    # DOCX files are ZIP archives containing XML — raw pattern scanning causes
    # false positives because XML legitimately contains tags like <object>, etc.
    # For DOCX/DOC: only check for macros and embedded executables (below).
    ...
    # Check for embedded executables — but only at the START of the file
    # (checking inside ZIP content causes false positives for DOCX)
    exe_sigs = [
        (b"MZ", "PE_executable"),
    ...
    # Only check the first 4 bytes for executable signatures (file header)
...
# Turnstile is enabled only when a real secret is set.
# Set TURNSTILE_SECRET_KEY="" or omit it entirely in .env to disable.
TURNSTILE_ENABLED = bool(TURNSTILE_SECRET) and TURNSTILE_SECRET.lower() not in ("disabled", "false", "none", "skip")

if not TURNSTILE_ENABLED:
    logger.info("[SECURITY] Cloudflare Turnstile is DISABLED — public endpoints will not require CAPTCHA")
else:
    logger.info("[SECURITY] Cloudflare Turnstile is ENABLED")


async def verify_turnstile(token: str, client_ip: str) -> bool:
    """Verify Cloudflare Turnstile CAPTCHA token.
    Returns True (pass) when Turnstile is disabled or verification succeeds.
    """
    if not TURNSTILE_ENABLED:
        return True  # Skip if not configured or explicitly disabled
    if not token:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not data.get("success"):
                ...
                await log_security_event("captcha_failed", client_ip, "MEDIUM",
                                         "Turnstile verification failed", {"errors": error_codes})
                logger.warning(f"[SECURITY] Turnstile failed for {client_ip}: {error_codes}")
                return False
            return True
    except Exception as e:
