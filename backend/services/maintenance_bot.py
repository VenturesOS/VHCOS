...
async def run_maintenance_cycle():
    ...
    # Stress mode with hysteresis: ON below 40, OFF above 60
    # Prevents oscillation when score hovers around 50
    if score < 40:
        await set_stress_mode(True)
    elif score > 60:
        await set_stress_mode(False)
    # Between 40-60: maintain current state (no change)
