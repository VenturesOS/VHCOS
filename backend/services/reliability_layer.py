...
async def set_stress_mode(active: bool):
    """Set stress mode with hysteresis to prevent oscillation.
    Turn ON when score < 40 (lower threshold).
    Turn OFF when score > 60 (higher threshold).
    Prevents rapid on/off cycling when score hovers around 50.
    """
    global _stress_mode
    if active == _stress_mode:
        return  # No change — don't log redundant events
    _stress_mode = active
    if active:
        await _log_event("stress_mode_on", "system", "LOW priority tasks paused (score < 40)")
    else:
        await _log_event("stress_mode_off", "system", "LOW priority tasks resumed (score > 60)")
