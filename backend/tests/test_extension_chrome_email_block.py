"""
Regression test for the Sachin/ajit@searchpartner.in bug.
...
Fix added a `snapshotChromeEmails()` helper + per-capture "chrome
blocklist" that mergeContacts() honours. We also restricted the
...
the blocklist plumbing silently. Version bump intentionally NOT asserted —
the fix is bundled into the next planned extension release, not shipped
as a standalone version bump.
"""
import re
