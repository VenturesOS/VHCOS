...
CF_ENFORCE = os.environ.get("CF_ACCESS_ENFORCE", "false").lower() in ("true", "1", "yes")

# M-07: Log enforcement status on import so misconfiguration is visible in startup logs
if CF_ENFORCE:
    logging.getLogger(__name__).info("[ZeroTrust] Cloudflare Access enforcement ENABLED")
else:
    logging.getLogger(__name__).warning(
        "[ZeroTrust] Cloudflare Access enforcement DISABLED. "
        "Set CF_ACCESS_ENFORCE=true in .env for production."
    )
ZERO_TRUST_ENABLED = bool(CF_TEAM_DOMAIN and CF_AUD) and CF_ENFORCE
