        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '48px' }}>
            {Array.from({ length: totalPages }, (_, i) => (
              <button key={i} onClick={() => setPage(i + 1)} style={{ width: '40px', height: '40px', borderRadius: '8px', border: page === i + 1 ? '2px solid #7CB342' : '1px solid #E5E7EB', background: page === i + 1 ? '#F0FDF4' : 'white', color: page === i + 1 ? '#7CB342' : '#6B7280', fontWeight: 600, cursor: 'pointer' }}>{i + 1}</button>
            ))}
          </div>
        )}
      </section>
        ...
        {blog.keywords?.length > 0 && (
          <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid #E5E7EB' }}>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {blog.keywords.map((k, i) => <span key={i} style={{ fontSize: '12px', background: '#F3F4F6', color: '#6B7280', padding: '4px 12px', borderRadius: '20px' }}>{k}</span>)}
            </div>
          </div>
        )}
      </article>
  ...
  return (
    <div className="min-h-screen bg-white" data-testid="candidate-blog-article">
      ...
      <article style={{ maxWidth: '760px', margin: '0 auto', padding: '48px 24px' }}>
        ...
        <div className="prose prose-lg prose-slate max-w-none" style={{ fontSize: '17px', lineHeight: 1.9, color: '#374151' }} dangerouslySetInnerHTML={{ __html: blog.content }} />
        {blog.keywords?.length > 0 && (
          <div style={{ marginTop: '48px', paddingTop: '32px', borderTop: '1px solid #E5E7EB' }}>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {blog.keywords.map((k, i) => <span key={i} style={{ fontSize: '12px', background: '#F3F4F6', color: '#6B7280', padding: '4px 12px', borderRadius: '20px' }}>{k}</span>)}
