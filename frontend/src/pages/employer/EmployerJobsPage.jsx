...
import { Search, Briefcase, MapPin, Clock, Users, Plus, Edit2, Trash2, Eye, Globe, GlobeLock, AlertTriangle, History, UserPlus, UserCheck, Link2, Copy, ExternalLink, Linkedin, Sparkles, LayoutGrid } from 'lucide-react';
  ...
  return (
    <div className="space-y-6" data-testid="employer-jobs-page">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-heading text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900">My Jobs</h1>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {job.status === 'active' && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-7 px-2 text-purple-600 border-purple-300 hover:bg-purple-50 gap-1.5"
                      ...
                      {job.assigned_recruiters?.length > 0 && (
                        <span 
                          ...
                          title={job.assigned_recruiter_names?.join(', ') || ''}
                        >
                          <UserCheck className="w-3 h-3" /> {job.assigned_recruiter_names?.length > 0 
                            ? job.assigned_recruiter_names.join(', ')
                            : `${job.assigned_recruiters.length} Assigned`}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-slate-500">
                      {job.posted_by_name && (
                        <span className="flex items-center gap-1" data-testid={`created-by-${job.id}`}>
                          <Users className="w-4 h-4" /> Created by: <span className="font-medium text-slate-700">{job.posted_by_name}</span>
                        </span>
                      )}
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    <LayoutGrid className="w-4 h-4 mr-1" /> View Pipeline
                  </Button>
