...
import { 
  ...
  TrendingDown, UserX, ChevronRight, Star, Edit2, Download, Eye, UserPlus
} from 'lucide-react';

// Stage configuration with colors and icons
const STAGES = {
  sourced: { label: 'Sourced', color: 'bg-slate-100 text-slate-700', icon: UserPlus },
...
export default function JobApplicantsPage() {
  ...
  const loadApplicants = useCallback(async (updateSelectedId = null) => {
    setLoading(true);
    try {
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-sm">
              <span className="text-amber-700">
                ...
                {applicant.last_edited_by.timestamp && (
                  <> on {new Date(applicant.last_edited_by.timestamp).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata' })}</>
                )}
              </span>
            </div>
          )}
