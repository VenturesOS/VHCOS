...
export default function EmployerCandidateBankPage() {
        ...
        <CardContent className="p-4 space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, email, phone, designation..." className="pl-10" onKeyDown={(e) => e.key === 'Enter' && handleSearch()} data-testid="search-input" />
            </div>
            <div className="flex gap-2">
              <Button onClick={() => setFiltersOpen(!filtersOpen)} variant="outline" className={activeFilterCount > 0 ? 'border-[#7CB342] text-[#7CB342]' : ''} data-testid="toggle-filters-btn">
                <Filter className="w-4 h-4 mr-1.5" /> Filters
                {activeFilterCount > 0 && <span className="ml-1.5 bg-[#7CB342] text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{activeFilterCount}</span>}
                {filtersOpen ? <ChevronUp className="w-4 h-4 ml-1" /> : <ChevronDown className="w-4 h-4 ml-1" />}
              </Button>
              <Button onClick={handleSearch} className="bg-[#7CB342] hover:bg-[#689F38]" data-testid="search-btn"><Search className="w-4 h-4 mr-1.5" /> Search</Button>
            </div>
          </div>
          {activeFilterCount > 0 && (
          ...
          {filtersOpen && (
            <div className="border-t pt-3 mt-1 space-y-3" data-testid="filter-panel">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Phone className="w-3 h-3" /> Phone</Label><Input value={filters.phone} onChange={(e) => updateFilter('phone', e.target.value)} placeholder="Search by phone" data-testid="filter-phone" className="text-sm" /></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Mail className="w-3 h-3" /> Email</Label><Input value={filters.email} onChange={(e) => updateFilter('email', e.target.value)} placeholder="Search by email" data-testid="filter-email" className="text-sm" /></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><MapPin className="w-3 h-3" /> Location</Label><Input value={filters.location} onChange={(e) => updateFilter('location', e.target.value)} placeholder="City or location" data-testid="filter-location" className="text-sm" /></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Building2 className="w-3 h-3" /> Company</Label><Input value={filters.company} onChange={(e) => updateFilter('company', e.target.value)} placeholder="Current company" data-testid="filter-company" className="text-sm" /></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Code className="w-3 h-3" /> Skills</Label><Input value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="Java, React, Python..." data-testid="filter-skills" className="text-sm" /></div>
                <div>
                  <Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Clock className="w-3 h-3" /> Notice Period</Label>
                  <Select value={filters.noticePeriod} onValueChange={(v) => updateFilter('noticePeriod', v === '_all' ? '' : v)}><SelectTrigger className="text-sm" data-testid="filter-notice"><SelectValue placeholder="Any" /></SelectTrigger><SelectContent><SelectItem value="_all">Any</SelectItem>{NOTICE_PERIODS.map(np => <SelectItem key={np} value={np}>{np}</SelectItem>)}</SelectContent></Select>
                </div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Briefcase className="w-3 h-3" /> Experience (years)</Label><div className="flex gap-1.5"><Input value={filters.minExperience} onChange={(e) => updateFilter('minExperience', e.target.value)} placeholder="Min" type="number" min="0" data-testid="filter-min-exp" className="text-sm" /><span className="text-slate-400 self-center text-xs">to</span><Input value={filters.maxExperience} onChange={(e) => updateFilter('maxExperience', e.target.value)} placeholder="Max" type="number" min="0" data-testid="filter-max-exp" className="text-sm" /></div></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><DollarSign className="w-3 h-3" /> Salary (INR)</Label><div className="flex gap-1.5"><Input value={filters.minSalary} onChange={(e) => updateFilter('minSalary', e.target.value)} placeholder="Min" type="number" data-testid="filter-min-salary" className="text-sm" /><span className="text-slate-400 self-center text-xs">to</span><Input value={filters.maxSalary} onChange={(e) => updateFilter('maxSalary', e.target.value)} placeholder="Max" type="number" data-testid="filter-max-salary" className="text-sm" /></div></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><EyeOff className="w-3 h-3" /> Contact Status</Label><Select value={filters.contactHidden} onValueChange={(v) => updateFilter('contactHidden', v === '_all' ? '' : v)}><SelectTrigger className="text-sm" data-testid="filter-contact-hidden"><SelectValue placeholder="Any" /></SelectTrigger><SelectContent><SelectItem value="_all">Any</SelectItem><SelectItem value="yes">Hidden (missing email/phone)</SelectItem><SelectItem value="no">Visible (has both)</SelectItem></SelectContent></Select></div>
                <div><Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><FileText className="w-3 h-3" /> Has Resume</Label><Select value={filters.hasResume} onValueChange={(v) => updateFilter('hasResume', v === '_all' ? '' : v)}><SelectTrigger className="text-sm" data-testid="filter-has-resume"><SelectValue placeholder="Any" /></SelectTrigger><SelectContent><SelectItem value="_all">Any</SelectItem><SelectItem value="yes">Yes</SelectItem><SelectItem value="no">No</SelectItem></SelectContent></Select></div>
                <div>
                  <Label className="text-xs text-slate-500 flex items-center gap-1 mb-1"><Calendar className="w-3 h-3" /> Captured</Label>
                  <div className="flex gap-1 mb-1.5">{[['Today','today'],['Week','week'],['Month','month']].map(([l,v]) => (<button key={v} onClick={() => setDatePreset(v)} className="px-2 py-0.5 text-xs rounded border border-slate-200 hover:border-[#7CB342] hover:text-[#7CB342] transition-colors" data-testid={`date-preset-${v}`}>{l}</button>))}</div>
                  <div className="flex gap-1.5"><Input value={filters.capturedAfter} onChange={(e) => updateFilter('capturedAfter', e.target.value)} type="date" data-testid="filter-date-after" className="text-sm" /><Input value={filters.capturedBefore} onChange={(e) => updateFilter('capturedBefore', e.target.value)} type="date" data-testid="filter-date-before" className="text-sm" /></div>
                </div>
              </div>
              <div className="flex justify-end gap-2 pt-1">
                <Button variant="ghost" size="sm" onClick={clearFilters} className="text-slate-500"><X className="w-3.5 h-3.5 mr-1" /> Clear</Button>
                <Button size="sm" onClick={handleSearch} className="bg-[#7CB342] hover:bg-[#689F38]"><Search className="w-3.5 h-3.5 mr-1" /> Apply Filters</Button>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={showUpload} onOpenChange={setShowUpload}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a resume to add a new candidate. Required fields: salary, notice period, location, experience.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Email (optional - will be extracted from resume)</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label>Name (optional - will be extracted from resume)</Label>
              <Input
      ...
      <Dialog open={showAddApplicant} onOpenChange={setShowAddApplicant}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Add this candidate to a job as an applicant. All fields are required.
            </DialogDescription>
          </DialogHeader>
          {applicantCandidate && (
            <div className="space-y-4">
              ...
              {(!editSalary || !editNotice || !editLocation || editExperience === '') && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2">
                  ...
                  <p className="text-sm text-amber-700">
                    All fields (salary, notice period, location, experience) are mandatory.
                  </p>
                </div>
              )}
            </div>
