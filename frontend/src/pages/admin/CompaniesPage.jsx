...
import {
  ...
  Phone, Mail, User, Layers, Percent, Hash, ExternalLink
} from 'lucide-react';
...
const EMPTY_COMMERCIAL = { type: 'percentage', percentage_value: '', fixed_fee_amount: '', level_config: [{ ...EMPTY_LEVEL }], fixed_fee_levels: [{ ...EMPTY_FIXED_LEVEL }] };
  ...
  return {
    ...
    commercial: {
      ...
      level_config: (comm.level_config?.length)
        ...
        : [{ ...EMPTY_LEVEL }],
      fixed_fee_levels: (comm.fixed_fee_levels?.length)
        ? comm.fixed_fee_levels.map(l => ({ level_name: l.level_name ?? '', min_salary: l.min_salary ?? '', max_salary: l.max_salary ?? '', fixed_fee: l.fixed_fee ?? '' }))
        : [{ ...EMPTY_FIXED_LEVEL }],
    },
  };
}
  ...
  if (comm.type === 'percentage') {
    ...
    comm.fixed_fee_levels = form.commercial.fixed_fee_levels
      ...
      .map(l => ({
        ...
        min_salary: parseFloat(l.min_salary),
        max_salary: parseFloat(l.max_salary),
        fixed_fee: parseFloat(l.fixed_fee),
      }));
  }
  return {
    name: form.name,
...
function CommercialSection({ commercial, onChange }) {
  ...
  const updateFixedLevel = (i, field, val) => {
    const next = [...(commercial.fixed_fee_levels || [{ ...EMPTY_FIXED_LEVEL }])];
    next[i] = { ...next[i], [field]: val };
    onChange({ ...commercial, fixed_fee_levels: next });
  };
  const addFixedLevel = () => onChange({ ...commercial, fixed_fee_levels: [...(commercial.fixed_fee_levels || []), { ...EMPTY_FIXED_LEVEL }] });
  const removeFixedLevel = (i) => {
    if ((commercial.fixed_fee_levels || []).length > 1) {
      onChange({ ...commercial, fixed_fee_levels: commercial.fixed_fee_levels.filter((_, idx) => idx !== i) });
    }
  };
  ...
  return (
    <div className="space-y-3" data-testid="commercial-section">
      ...
      {/* Type Selector */}
      <div className="flex gap-2 flex-wrap" data-testid="commercial-type-selector">
        {[
          { key: 'percentage', label: 'Flat %', icon: Percent },
          { key: 'fixed', label: 'Fixed Fee', icon: Hash },
          { key: 'level_based', label: 'Level % Based', icon: Layers },
          { key: 'fixed_level_based', label: 'Level Fixed Fee', icon: Layers },
      ...
      {/* Fixed Fee Level-Based Config */}
      {commercial.type === 'fixed_level_based' && (
        <div className="space-y-2" data-testid="commercial-fixed-level-config">
          <div className="grid grid-cols-5 gap-2 text-xs text-gray-500 font-medium px-1">
            <span>Level Name</span>
            <span>Min Salary</span>
            <span>Max Salary</span>
            <span>Fixed Fee (INR)</span>
            <span></span>
          </div>
          {commercial.fixed_fee_levels.map((lv, i) => (
            <div key={i} className="grid grid-cols-5 gap-2 items-center" data-testid={`fixed-level-row-${i}`}>
              <Input
                type="text" placeholder="e.g. Junior"
                value={lv.level_name}
                onChange={e => updateFixedLevel(i, 'level_name', e.target.value)}
                data-testid={`fixed-level-name-${i}`}
              />
              <Input
                type="number" min="0" placeholder="100000"
                value={lv.min_salary}
                onChange={e => updateFixedLevel(i, 'min_salary', e.target.value)}
                data-testid={`fixed-level-min-${i}`}
              />
              <Input
                type="number" min="0" placeholder="1200000"
                value={lv.max_salary}
                onChange={e => updateFixedLevel(i, 'max_salary', e.target.value)}
                data-testid={`fixed-level-max-${i}`}
              />
              <Input
                type="number" step="1" min="0" placeholder="50000"
                value={lv.fixed_fee}
                onChange={e => updateFixedLevel(i, 'fixed_fee', e.target.value)}
                data-testid={`fixed-level-fee-${i}`}
              />
              <div className="flex justify-end">
                {commercial.fixed_fee_levels.length > 1 && (
                  <Button type="button" variant="ghost" size="icon" className="text-red-400 hover:text-red-600" onClick={() => removeFixedLevel(i)} data-testid={`fixed-level-remove-${i}`}>
                    <Trash2 className="w-4 h-4" />
                  </Button>
                )}
              </div>
          ...
          <Button type="button" variant="outline" size="sm" onClick={addFixedLevel} data-testid="add-fixed-level-btn">
            <Plus className="w-3.5 h-3.5 mr-1" /> Add Level
          </Button>
        </div>
      )}
...
function CommercialBadge({ commercial }) {
  ...
  if (commercial.type === 'fixed_level_based') {
    const levels = commercial.fixed_fee_levels || [];
    if (levels.length > 0) {
      return <Badge className="bg-indigo-50 text-indigo-700 border-indigo-200">Fixed Levels: {levels.length} tiers</Badge>;
    }
    return <Badge variant="outline" className="text-gray-400">Fixed Level-based (empty)</Badge>;
  }
  return null;
}
            ...
            <Card key={company.id} className="hover:shadow-md transition-shadow" data-testid={`company-card-${company.id}`}>
              <CardContent className="p-5">
                ...
                <div className="flex gap-2 pt-2 border-t border-gray-100">
                  <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/company-profile/${company.id}`)} data-testid={`view-profile-${company.id}`}>
