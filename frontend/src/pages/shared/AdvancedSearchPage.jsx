        ...
        <Card className="border-slate-200" data-testid="search-results">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-slate-100">
              {results.map((c, idx) => (
                <div key={c.id ? `${c.id}-${idx}` : `result-${idx}`} className="p-4 hover:bg-slate-50 transition-colors" data-testid={`result-${c.id || idx}`}>
                  ...
                  {c.smart_tags?.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {c.smart_tags.slice(0, 5).map((tag, ti) => (
                        <span key={`${c.id}-tag-${tag}-${ti}`} className="px-1.5 py-0.5 text-[10px] bg-indigo-50 text-indigo-600 rounded border border-indigo-100 font-medium">{tag}</span>
                      ))}
                  ...
                  {c.skills?.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {c.skills.slice(0, 6).map((s, si) => <span key={`${c.id}-skill-${s}-${si}`} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-xs rounded-full">{s}</span>)}
