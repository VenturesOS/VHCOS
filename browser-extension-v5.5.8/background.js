...
function computeMatchScore(card, hist) {
  // ── Signal 0: Naukri ID match (definitive ONLY when equal) ──
  // Naukri rotates these IDs every few hours, so a *mismatch* is NOT
  // evidence that the people are different. A match is still a strong
  // positive signal though. We treat mismatch as "no signal" and fall
  // through to multi-signal scoring.
  const cardNaukriId = card.naukri_id || null;
  const histNaukriId = hist.profileId || hist.naukri_profile_id || null;
  if (cardNaukriId && histNaukriId && cardNaukriId === histNaukriId) {
    return 100;
  }
...
function postValidateApiResults(candidates, apiResults, localHistory) {
  ...
  return apiResults.map(result => {
    ...
    // The /api/extension/check-existing endpoint echoes the DB candidate's
    // key fields. We use this to reject obvious CONFLICTS only — not to
    // re-prove the match. (Server has already done multi-signal scoring.)
    //
    // Reject when:
    //   • Both employer fields present AND clearly different (no substring overlap)
    //   • Both experience_years present AND > 3 years apart
    //   • Both naukri_ids present, equal — actually a confirm signal, never reject
    // Note: We do NOT reject on naukri_id mismatch — Naukri rotates these
    // IDs every few hours, so a mismatch is not evidence of different people.
    if (result.matched_candidate) {
      const mc = result.matched_candidate;

      // Confirm signal: identical Naukri IDs (rare but definitive)
      if (card.naukri_id && mc.naukri_profile_id && card.naukri_id === mc.naukri_profile_id) {
        return { ...result, match_confidence: 'high' };
      }

      // Conflict 1: employer mismatch (when both visible)
      const empClean = (s) => (s || '').toLowerCase()
        .replace(/\b(pvt|ltd|llp|inc|corp|limited|private|co|company|india)\b/g, '')
        .replace(/[^a-z0-9 ]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      const ce = empClean(card.current_employer);
      const me = empClean(mc.current_employer);
      if (ce && me && ce.length > 2 && me.length > 2 &&
          !ce.includes(me) && !me.includes(ce)) {
        console.log(
          `[VHC BG v${VERSION}] Post-validation REJECTED (employer conflict): ` +
          `card="${card.name}" emp="${card.current_employer}" ` +
          `≠ db="${mc.name}" emp="${mc.current_employer}"`
        );
        return { index: result.index, exists: false };
      }

      // Conflict 2: experience > 3 years apart
      if (card.experience_years != null && mc.experience_years != null) {
        const expDiff = Math.abs(Number(card.experience_years) - Number(mc.experience_years));
        if (expDiff > 3) {
          console.log(
            `[VHC BG v${VERSION}] Post-validation REJECTED (experience conflict): ` +
            `card="${card.name}" exp=${card.experience_years}y vs db exp=${mc.experience_years}y`
          );
          return { index: result.index, exists: false };
        }
      }

      // No conflicts — preserve server's confidence (high/medium) for the badge
      return result;
    }

    // ── Legacy fallback: try to find matched candidate in local history ──
    // (used when the server hasn't sent matched_candidate, i.e. older backend)
