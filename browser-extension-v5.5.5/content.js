...
(function() {
  ...
  function markCardAsExisting(cardEl, info) {
    ...
    if (nameEl && !cardEl.querySelector('.vhc-existing-badge')) {
      ...
      badge.title = `Saved on ${info.captured_at ? new Date(info.captured_at).toLocaleDateString() : 'unknown date'} — click to open in VHC`;
      badge.target = '_blank';
      badge.rel = 'noopener noreferrer';
      // href is set asynchronously once we resolve the frontend base URL
      badge.href = '#';
      badge.dataset.vhcCandidateId = info.candidate_id || '';

      // Resolve and assign the deep-link URL from background (knows api+frontend base)
      ...
      badge.addEventListener('click', (e) => {
        e.stopPropagation();
        // Fallback: if href is still "#" (background slow), open via message
